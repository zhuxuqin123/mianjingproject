...
<script>
import { articleDetailAPI, likecollectAPI } from '@/API/API'
export default {
  name: 'detailPage',
  data () {
    return {
  ...
  async created () {
    const res = await articleDetailAPI(this.$route.params)
    console.log(res)
  ...
  methods: {
    // 点赞
    async toggleLike () {
      await likecollectAPI({ id: this.$route.params.id, optType: 1 })
    ...
    // 收藏
    async toggleCollect () {
      await likecollectAPI({ id: this.$route.params.id, optType: 2 })
